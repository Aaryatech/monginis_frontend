	...
	<div class="fullGrid center">
		...
		<div class="wrapperIn2">
			...
			<div class="sidebarright">
				...
				<br> <br> <br>
				<form action="${pageContext.request.contextPath}/orderRegularSpCake"
					...
					<div class="ordercake">
							...
							<div class="fullform">
								...
								<div class="cackright">
									<select name="regular_sp_cake" id="regular_sp_cake"
										class="form-control" onchange="subCatOnChange();" required>
										<option value="">Select Category</option>

										<c:forEach items="${categoryResponse}"
											var="mCategoryWithSubCat">
											<option value="${mCategoryWithSubCat.subCatId}"><c:out
													value="${mCategoryWithSubCat.subCatName}" /></option>
										</c:forEach>
									</select>
								</div>
							</div>
						...
						<input type="hidden" id="t1" name="t1" value=""> <input
							...
							value="${frDetails.getFrRateCat()}"> <input type="hidden"
							id="rate" name="rate" value=""> <input type="hidden"
							id="MRP" name="MRP" value="">
				</form>
				<!--rightForm-->
			...
			.ready(
					function() {
						$('#regular_sp_cake')
								.change(
										function() {
											$('#regSpCkItem').find('option')
													.remove().end()
											if ($(this).val() != 14) {
												document
														.getElementById("sp_qty").readOnly = true;
											} else {
												document
														.getElementById("sp_qty").readOnly = false;
											}
											$
													.getJSON(
															'${findAllRegularSpCk}',
											...
											$
													.getJSON(
															...
															function(data) {
																...
																var mrpBaseRate = parseFloat(mrp
																		/ (taxPer3 + 100));
																...
																if (taxPer3 == 0) {
																	...
																	if (taxPer1 == 0) {
																		...
																		taxPerPerc1 = parseFloat((taxPer1 * 100)
																				/ taxPer3);
																	...
																	if (taxPer2 == 0) {
																		...
																		taxPerPerc2 = parseFloat((taxPer2 * 100)
																				/ taxPer3);
...
function qtyOnChange(){
	...
	if(${index}==7){
		...
		if (document.getElementById("regular_sp_cake").value == 14) {
			...
			if(qty.length>=2){
				if(qty<36){
					alert("Minimum quantity should be 36");
				}else{
					calculatePerQuantity();
				}
			}else{
...
<script type="text/javascript">
	function calculatePerQuantity() {
		...
		$("#price").text(total);
		document.getElementById("sp_calc_price").setAttribute('value', total);

		$("#subtotal").text(total);
		document.getElementById("sp_sub_total").setAttribute('value', total);

		$("#INR").text('INR-' + total);
		document.getElementById("sp_grand").setAttribute('value', total);
		...
		document.getElementById("total_amt").setAttribute('value', total);

		$('#mgstamt').html('AMT-' + mrpBaseRate.toFixed(2));
		document.getElementById("m_gst_amt").setAttribute('value',
				mrpBaseRate.toFixed(2));

		$('#rmAmt').html(total);
		document.getElementById("rm_amount").setAttribute('value', total);
...
<script type="text/javascript">
	function subCatOnChange() {
		//If pastries then set quantity >36
		//alert("val - " + document.getElementById("regular_sp_cake").value);
